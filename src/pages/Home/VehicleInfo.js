...
export default function VehicleInfo({data}) {
  return (
    ...
    <div className="flex flex-wrap items-center mb-4">
      ...
      <div>
        <h3 className="text-[#151547] text-base font-medium tracking-[-0.05em] uppercase">{data?.driver_id?.name}</h3>
    ...
    <div className="flex flex-wrap ">
      <div className="w-4/12 pr-1 flex">
        <div>
          ...
          <p className="text-[#666666] text-[12px] font-medium tracking-[-0.04em] mb-0 capitalize">{data?.driver_id?.company_name}</p>
        </div>
      </div>
      <div className="w-4/12 pr-1 pl-1 text-center border-r border-l border-black border-opacity-10 flex justify-center ">
        <div>
          ...
          <p className="text-[#666666] text-[12px] font-medium tracking-[-0.04em] mb-0">{data?.driver_id?.email}</p>
        </div>
      </div>
      <div className="w-4/12 pr-1 flex justify-end">
        <div>
          <h3 className="text-black  tracking-[-0.04em] text-sm font-medium m-0 ">Contact</h3>
          <p className="text-[#666666] text-[12px] font-medium tracking-[-0.04em] mb-0">{data?.driver_id?.contact}</p>
        </div>
      </div>
    </div>
